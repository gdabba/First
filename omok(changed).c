							...
							if(!q){
								...
								while(q3){
									...
									case 'N':case 'n':
										...
										sendXY(0,0,socket);
										clear();
										mvaddstr(0,0,"press N. out from room.                       ");
										sleep(2);
										refresh();
										q3 = 0;
										backtomenu = 1;
...
int isVictory3(char MSCc){
 
	int tx, ty;
 
	for(tx=0; tx<5; tx+=2)
	{
		if(map_mirror3[0][tx] == MSCc)	
			if(map_mirror3[1][tx] == MSCc)
				if(map_mirror3[2][tx] == MSCc)
					return 1;
	}
 
	for(ty=0; ty<3; ty++)
	{
		if(map_mirror3[ty][0] == MSCc)	
			if(map_mirror3[ty][2] == MSCc)
				if(map_mirror3[ty][4] == MSCc)
					return 1;
	}
 
	if(map_mirror3[0][0] == MSCc)
		if(map_mirror3[1][2] == MSCc)
			if(map_mirror3[2][4] == MSCc)
				return 1;
 
	if(map_mirror3[0][4] == MSCc)
		if(map_mirror3[1][2] == MSCc)
			if(map_mirror3[2][0] == MSCc)
				return 1;
...
void start(){
	...
	while(q){
		...
		case '1':
			clear();
			mvaddstr(20,0,"                                    1. Play Sam mok");
			mvaddstr(21,0,"                                    2. Play Sa mok");
			mvaddstr(22,0,"                                    3. Play O mok");
